...
import { showBrideEvent, showGroomEvent } from "../lib/invitationView";
...
export default function Events({
  ...
  const showGroom = showGroomEvent(side);
  const showBride = showBrideEvent(side);
  const singleSide = !showGroom || !showBride;
  ...
  return (
    <Section id="events">
      <div className="w-full">
        <div
          className={[
            "flex w-full min-w-0 flex-col items-stretch gap-6 md:h-[706px] md:flex-row md:items-stretch md:gap-5 lg:gap-6",
            singleSide ? "md:justify-center" : "",
          ].join(" ")}
        >
          {showGroom ? (
            <div
              className={[
                "relative w-full min-w-0 flex-1 overflow-hidden rounded-[30px] md:h-[706px] md:min-h-0",
                singleSide ? "md:max-w-[640px]" : "",
              ].join(" ")}
            >
              <img
                alt=""
